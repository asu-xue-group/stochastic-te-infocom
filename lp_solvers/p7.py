...
def solve_p7(G: SrgGraph, k, gamma, p: list):
    with gp.Env(empty=True) as env:
        ...
        m.addConstrs((gp.quicksum(W[i, r] for r in R) >= gamma * commodities[i].demand for i in I), name='B1')
        ...
        m.addConstrs((gp.quicksum(W[i, r] * L(l, paths[i][r], e) * g[e[0]][e[1]]['cost']
                                  for e in E for r in R) <= commodities[i].budget for i in I), name='B3')
